...
describe("REST libreria", function () {
  ...
  describe("Clientes", function () {
    ...
    it(`GET ${URL}/clientes?email=...`, async () => {
      ...
      request = requester.get(`${URL}/clientes`).query({ email: 'buscar@test.com' });
      let response = await request.send();
      assert.equal(response.status, 200);
      assert.equal(response.body.email, 'buscar@test.com');
      requester.close();
    });
    ...
    it(`DELETE ${URL}/clientes/:id`, async () => {
      let requester = chai.request.execute(app).keepOpen();
      let cliente = crearCliente('eliminar@test.com');
  ...
  describe("Administradores", function () {
    ...
    it(`POST ${URL}/admins`, async () => {
      let requester = chai.request.execute(app).keepOpen();
      let admin = crearAdmin('admin@test.com');
    ...
    it(`POST ${URL}/admins/autenticar`, async () => {
      let requester = chai.request.execute(app).keepOpen();
      let admin = crearAdmin('adminlogin@test.com');
    ...
    it(`PUT ${URL}/admins/:id`, async () => {
      let requester = chai.request.execute(app).keepOpen();
      let admin = crearAdmin('adminupdate@test.com');
    ...
    it(`DELETE ${URL}/admins/:id`, async () => {
      let requester = chai.request.execute(app).keepOpen();
      let admin = crearAdmin('admindelete@test.com');
